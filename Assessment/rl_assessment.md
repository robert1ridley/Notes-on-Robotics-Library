...
我认为Robotics Library的许多体系结构决策都是基于灵活性的。对于开发者来说，有必要在不影响现有功能的情况下添加新功能。例如，开发者想添加计算机视觉组件的话，该组件的逻辑需要独立的，不要跟其他组件的逻辑混在一起。我相信正是这种需求导致了选择基于组件的分层体系结构。这使得添加新的组件不需要太多麻烦。使用一些简单的设计模式（如工厂模式）的选择允许在编码功能和使用库方面具有更大的灵活性。
